...
- **单通道失败不会阻止另一通道继续执行**：当前脚本会记录失败并继续跑下一个通道，而不是整批次提前退出
...
4. 如果只是某一个 channel 失败，先按单通道问题处理；当前脚本默认会继续运行另一个 channel
...
当前脚本行为说明：
- 单个 channel 失败时，**不会阻止另一个 channel 继续运行**
- 因此应优先按“单通道故障”处理，而不是默认整批次失败
